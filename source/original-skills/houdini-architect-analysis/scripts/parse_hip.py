...
Args:
    ...
    subnet_path:    OPTIONAL relative path inside extracted_root to start dumping
                    from (default: obj/<first geo node>). Example: "obj/box1"
...
def find_default_subnet(extracted_root):
    """Find a sensible default subnet path: first geo node under obj/."""
    obj_dir = extracted_root / "obj"
    if not obj_dir.is_dir():
        return extracted_root  # fall back to root
    for child in sorted(obj_dir.iterdir()):
        if child.is_dir():
            return child
        if "." in child.name:
            base, ext = child.name.rsplit(".", 1)
            if ext == "def":
                sub = obj_dir / base
                if sub.is_dir():
                    return sub
