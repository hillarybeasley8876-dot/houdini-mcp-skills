...
- **PCG 关键**: [[houdini-pcg-practitioner]] 的 5-stage 流水线,每个 stage 都是一个 subnet
...
- **PCG 核心**: [[houdini_procedural_modeling]] 几乎所有 pattern 都是"用 attribute 编码语义,下游按 attribute 分支" — 这就是 Houdini 的"协议"
...
- **本章后续直接深拆 4 件套的子页**:
  - [User interface](02_userinterface.md) — 怎么操作上述抽象
  - [Networks and parameters](08_networks.md) — 网络结构详解
  - [Cooking](13_cooking.md) — DAG 求值的细节
- **跨章引用**:
...
2. **遇到数据先问"它绑在哪个层级"**(detail/prim/point/vertex,见 [[houdini_procedural_modeling]] 第 5 节)
3. **遇到重复需求先问"能不能 HDA 化"**,不要复制网络
4. **遇到性能问题先看 cook 树**,不要瞎调参数

**跟现有资产的关联**:
- [[houdini-pcg-practitioner]] 的 5-stage 流水线 = 4 件套的工程化运用
- [[houdini_procedural_modeling]] 36 个 VEX pattern = 4 件套的高级用法
- [[houdini-architect-analysis]] = 反推他人怎么用 4 件套搭 .hip

**读完这一节我应该能做到**: 看到一个 .hip 文件能用 4 概念把它分类(网络结构 + 节点类型 + 属性流 + HDA 边界),不被术语吓到。

## Cross-references

- 下一节: [User interface](02_userinterface.md)
- 4 件套深拆位置:
...
- 相关 wrangle pattern: [[houdini_procedural_modeling]]
- Practitioner playbook: [[houdini-pcg-practitioner]]
