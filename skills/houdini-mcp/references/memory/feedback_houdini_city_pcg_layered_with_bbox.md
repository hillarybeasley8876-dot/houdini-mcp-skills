...
4. **建筑细节层** — 在每个 bbox 内,按 [houdini_procedural_modeling](houdini_procedural_modeling.md) 36 patterns(湖边小屋方法)生成单栋建筑
...
- 落地手册在 skill [houdini-pcg-city-pipeline](../../../houdini-pcg-city-pipeline/SKILL.md),该 skill 把这 4 层作为 canonical organization
- 下层细节方法库在 [houdini_procedural_modeling](houdini_procedural_modeling.md) 的 36 patterns
- 跨尺度 PCG 的目标是符合 [houdini_pcg_practitioner_goal](houdini_pcg_practitioner_goal.md) 的"做出来"水平
- 腾讯 ref 文档 [reference-pcg-city-template](reference_pcg_city_template.md) 是"上层做得不错但下层是空的"的反例,刚好是这个架构需要补的下层
